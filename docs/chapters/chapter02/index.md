...
[第1回](https://zenn.dev/link-to-part1)では、計算論的物理主義の概要を提示しました。本稿では、その理論的基盤である計算可能性理論を用いて、「人間の知性は原理的にAIによって模倣可能である」という主張を厳密に証明します。
...
ここでの `O(1)` は、「入力サイズが変わってもほとんど処理時間が変わらないような、ごく単純な処理」（例：固定のテンプレートに値を埋め込むだけの処理）をイメージしてください。一方で `O(exp(n))` のような表現は、「入力が少し増えるだけで必要な計算時間が爆発的に増えるような探索問題」を表現しており、本章ではこの違いを手がかりに知性の階層を捉え直しています。
...
**応答**：
本理論は機能主義的立場を取る。重要なのは入出力の一致であり、内的体験の同一性は工学的には無関係。
