---
layout: book
title: "第2章：計算可能性理論に基づく理論的検討"
---

# 第2章：計算可能性理論に基づく理論的検討

## はじめに

[第1章](../chapter01/)では、計算論的物理主義の概要を提示しました。本章では、その理論的基盤である計算可能性理論を用いて、「人間の知性は原理的にAIによって模倣可能である」という主張を、計算可能性理論の枠組みで可能な範囲で厳密に示します（機能主義などの哲学的立場自体を数学的に証明することが目的ではありません）。
...
### 物理的チャーチ＝チューリングのテーゼ（本書の射程）

第1章で述べた通り、本書は **Modest（控えめ）Physical CTT** を前提に議論します。つまり、観測・制御可能な有限精度の物理過程を、工学的な計算モデルで近似できる、という射程に限定します。

ここで区別しておくべき点は次の2つです。

- **計算可能性（computability）**：そもそも「解ける/計算できる」問題か
- **効率（complexity）**：どれだけの資源（時間・空間・エネルギー）で解けるか

量子計算は、主として後者（効率）に影響し得る一方で、前者（計算可能性）を一般に「拡張する」と断定できるわけではありません。本書は、計算可能性と効率を混同せず、「有限資源の中で何が現実的に実装・運用できるか」を中心に扱います。
...
注：ここでの「有限性」は、観測・制御・記録が有限精度であり、かつ有限資源（時間・エネルギー）で動くという工学的前提に基づく理想化です。連続値や量子のような議論はあり得ますが、本書では「実務で扱う有限精度のモデル化」に射程を絞ります。
...
この `∀入力` は理想化です。実務では入力空間が巨大・無限であるため、次のように扱います。

- **分布（D）を置く**：運用で想定する入力分布を定義する（何を想定していないかも明記する）
- **サンプルで検証する**：有限個のテスト入力で差分を評価する
- **統計的保証（PAC的）として読む**：サンプル数と誤差・信頼度の関係として扱う（全入力保証ではない）

つまり「ブラックボックス等価性」は、数学的には強い定義を置きつつ、運用上は**分布・サンプル・保証の範囲**に落として使う、という立て付けになります。

### 実装：等価性検証アルゴリズム

以下は概念的な擬似コードです（そのまま実装可能な完全な検証器を提供することが目的ではありません）。
```python
