...
Physical CTT は「物理的に実現可能な計算過程」を計算モデルで扱える、という主張の総称です。[^sep-computation-physicalsystems] 本書は次の **Modest（控えめ）** な形を作業仮説として採用します。

- **Bold（強い）**：あらゆる物理過程が（効率も含めて）計算機でシミュレート可能、等の強い主張まで含み得る
- **Modest（控えめ）**：観測・制御可能な物理過程を、有限精度・有限時間の範囲で計算モデルに写像して議論する

連続系・超計算・非標準量子などが「計算として利用可能」な形で成立する場合、Physical CTT（少なくとも Bold）は否定され得ます。本書はその可能性を排除して「証明」するのではなく、以後の議論がどの前提に依存しているかを明示します。
...
[^sep-church-turing]: SEP: The Church-Turing Thesis. `[@sep-church-turing]`（参考リンクは `references/bibliography.md`）  
  [plato.stanford.edu/entries/church-turing](https://plato.stanford.edu/entries/church-turing/)
[^sep-computation-physicalsystems]: SEP: Computation in Physical Systems. `[@sep-computation-physicalsystems]`（参考リンクは `references/bibliography.md`）  
...
[^sep-qt-quantcomp]: SEP: Quantum Computing. `[@sep-qt-quantcomp]`（参考リンクは `references/bibliography.md`）  
