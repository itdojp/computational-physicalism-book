...
第1章〜第3章で、計算論的物理主義の理論的基盤を整理しました。本章では、この理論を実際のAI開発にどう接続するかという観点から、具体的な実装戦略を提示します。
...
> **主張区分**: `数学的・形式的主張` / `思考実験・擬似コード` / `編集上の解釈`
...
> **主張区分**: `思考実験・擬似コード` / `仮説・将来予測` / `編集上の解釈`
...
以下のロードマップは、議論の整理に用いる**反証可能なシナリオ例**であり、技術到達時期の予測ではありません。`t+n`と計算量の数値は、依存関係と桁差を検討するための仮置きです。実際の暦年、予算、調達計画には対応付けません。各段階の評価指標、対象システム、資源上限を固定した検証結果が得られた場合に更新し、前段の成果が再現できない場合は後段のシナリオを棄却します。
...
def medium_term_goals():
    return {
        "compute_scaling": {
            "t+5": 1e20,   # 感度分析用の仮置き。実測・予測値ではない
            "t+8": 1e23,
            "t+10": 1e25   # 感度分析用の仮置き。実測・予測値ではない
        },
        "capabilities": {
            "t+6": "専門家レベルの問題解決",
            "t+9": "創造的タスクの自動化",
...
計算論的物理主義は、単なる理論的考察に留めず、具体的な実装戦略として検討できる論点を含みます。本章の提案は、工学的に検証可能な仮説と評価ハーネスへ落とし込むための出発点です。

重要なのは、この理論が示す未来は決定論的ではないということです。人間とAIがどのように共進化するかは、私たちがどのような実装選択をするかに依存します。

本書の前提に立てば、人間レベルの知性を計算可能な目標として扱える可能性があります。しかし、その実現可能性、評価方法、社会への導入方法は、今後の研究・実装・制度設計に依存します。
...
計算論的物理主義の検証と安全な応用に向けて、前提と限界を明示しながら歩みを進めましょう。
