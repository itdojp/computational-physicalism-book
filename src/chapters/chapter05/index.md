...
しかし、これらの能力の一部ではAIが人間を上回る場面が増えている。計算論的物理主義の観点からは、定型的なパターン処理において専用に設計されたシステムが汎用システムを上回り得る、と説明できる。
...
NIST AI RMF は、法律ではなくリスク管理フレームワークです。運用の観点では、次の4機能を**継続プロセス**として回すことが重要です。
...
AIの能力は、学習時計算量だけでなく推論時計算量にも依存します。しかし推論時計算量を増やす運用（再試行、探索、外部ツール連携）は、そのまま**電力・コスト・待ち時間**を増やします（例：IEA, 2025）。
